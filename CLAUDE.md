...
Both implementations expose the same `EngineService` API (declared via `engine_types.dart`): `evaluationStream`, `analyzePosition(fen)`, `evaluatePosition(fen, depth:)`, `stop()`, `dispose()`. When adding or changing engine behavior, update **both** `_native` and `_web` files and keep types defined in `engine_types.dart`. Stockfish is provided by the `stockfish` pub package on native; web has its own path.
